<x-filament-panels::page>
    ...
    @else
    @php
        $lineSupplierFilter = (string) ($this->lineSupplierFilter ?? '');
        $supplierFilterOptions = collect($lines)
            ->map(fn ($line) => [
                'id' => (string) ($line['supplier_id'] ?? ''),
                'name' => (string) ($line['supplier_name'] ?? '-'),
            ])
            ->filter(fn ($supplier) => $supplier['id'] !== '')
            ->unique('id')
            ->sortBy('name')
            ->values();
        $visibleLines = collect($lines)
            ->filter(fn ($line) => $lineSupplierFilter === '' || (string) ($line['supplier_id'] ?? '') === $lineSupplierFilter);
                ...
                <div class="flex flex-wrap items-center justify-end gap-3">
                    <div class="flex flex-wrap items-center gap-2 rounded-md bg-slate-50 px-3 py-2 text-sm font-bold text-slate-700 dark:bg-gray-800 dark:text-gray-200">
                        ...
                        <span class="inline-flex items-center gap-1 rounded-md border border-emerald-200 bg-emerald-50 px-2 py-1 text-emerald-800 dark:border-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200">
                            表示合計
                            <span class="font-mono text-lg font-bold text-emerald-950 dark:text-emerald-100">¥{{ number_format($visibleLineTotalAmount, 2) }}</span>
                        </span>
                    </div>
                    <div>
                        <select
                            wire:model.live="lineSupplierFilter"
                            class="w-72 rounded-md border-slate-300 px-3 py-2 text-sm font-semibold shadow-sm dark:border-gray-600 dark:bg-gray-900"
                        >
                            <option value="">仕入先すべて</option>
            ...
            <div class="min-h-0 flex-1 overflow-auto">
                <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-gray-700">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-gray-800">
                            ...
                            <tr wire:key="registration-line-{{ $index }}" class="odd:bg-[#f5f9ff] even:bg-white dark:odd:bg-[#1e2a3b] dark:even:bg-gray-900">
                                ...
                                <td class="whitespace-nowrap px-3 py-2">
                                    <div class="flex items-center gap-2">
                                        <div>
                                            <div class="font-semibold text-slate-800 dark:text-gray-100">{{ $line['supplier_name'] ?? '-' }}</div>
                                        ...
                                        <button
                                            type="button"
                                            wire:click="openSupplierChangeModal({{ $index }})"
                                ...
                                <td class="whitespace-nowrap px-3 py-2 text-right font-mono">¥{{ number_format($purchaseUnitPrice, 2) }}</td>
                                <td class="whitespace-nowrap px-3 py-2 text-right font-mono font-bold text-emerald-700 dark:text-emerald-300">¥{{ number_format($lineTotalAmount, 2) }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="14" class="px-3 py-12 text-center text-sm text-slate-400 dark:text-gray-500">登録リストは空です</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    @if ($showSupplierChangeModal)
        @php
            $targetLine = $supplierChangeLineIndex !== null && isset($lines[$supplierChangeLineIndex])
                ? $lines[$supplierChangeLineIndex]
                : null;
        @endphp
        <div class="fixed inset-0 flex items-center justify-center bg-slate-950/50 p-4" style="z-index: 10000;">
            <div class="flex h-[92vh] max-h-[92vh] flex-col overflow-hidden rounded-lg bg-white shadow-xl dark:bg-gray-900" style="width: min(1120px, 96vw);">
                <div class="flex items-center justify-between bg-slate-800 px-4 py-3 text-white">
                    ...
                    <button type="button" wire:click="closeSupplierChangeModal" class="rounded p-1 text-white hover:bg-white/10">
                        <x-heroicon-o-x-mark class="h-5 w-5" />
                    </button>
                </div>
                        ...
                        <div class="rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm dark:border-gray-700 dark:bg-gray-800">
                            <div class="flex flex-wrap gap-x-6 gap-y-1">
                                ...
                                <div>
                                    ...
                                    <span class="ml-1 font-semibold text-slate-900 dark:text-white">{{ $targetLine['supplier_name'] ?? '-' }}</span>
                                </div>
                            </div>
                        </div>
                    @endif

                    <form wire:submit.prevent="searchSupplierChangeCandidates" class="rounded-md border border-slate-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
                        <div class="flex flex-wrap items-end gap-2">
                            <label class="min-w-80 flex-1">
                                <span class="mb-1 block text-xs font-semibold text-slate-600 dark:text-gray-300">仕入先検索</span>
                                <input
                                    type="text"
                                    wire:model.defer="supplierChangeSearch"
                        ...
                        <div class="min-h-0 flex-1 overflow-auto">
                            <table class="min-w-full divide-y divide-slate-200 text-sm dark:divide-gray-700">
                                <thead class="sticky top-0 z-10 bg-slate-50 text-xs text-slate-600 dark:bg-gray-800 dark:text-gray-300">
                                    <tr>
                                        <th class="whitespace-nowrap px-3 py-2 text-left">仕入先CD</th>
                                        <th class="min-w-72 px-3 py-2 text-left">仕入先名</th>
                                ...
                                <tbody class="divide-y divide-slate-100 dark:divide-gray-800">
                                    @forelse ($supplierChangeRows as $row)
                                        <tr class="odd:bg-[#f5f9ff] even:bg-white dark:odd:bg-[#1e2a3b] dark:even:bg-gray-900">
                                            <td class="whitespace-nowrap px-3 py-2 font-mono text-xs">{{ $row['supplier_code'] ?: $row['supplier_id'] }}</td>
                                            <td class="px-3 py-2 font-semibold text-slate-800 dark:text-gray-100">{{ $row['supplier_name'] }}</td>
                                            <td class="whitespace-nowrap px-3 py-2">
                                                @if (($row['contractor_count'] ?? 0) === 1)
                                                    <span class="font-mono text-xs">{{ $row['contractor_code'] }}</span>
                                                    <span class="ml-1">{{ $row['contractor_name'] }}</span>
                                                @elseif (($row['contractor_count'] ?? 0) > 1)
                                                    <span class="font-semibold text-danger-700 dark:text-danger-300">発注先複数</span>
                                                @else
                                                    <span class="font-semibold text-slate-400 dark:text-gray-500">発注先未設定</span>
                                                @endif
                                            </td>
                                            <td class="whitespace-nowrap px-3 py-2 text-center">
                                            ...
                                            <td class="whitespace-nowrap px-3 py-2 text-center">
                                                <button
                                                    type="button"
                                                    wire:click="applySupplierChange({{ (int) $row['supplier_id'] }})"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-3 py-10 text-center text-sm text-slate-400 dark:text-gray-500">仕入先が見つかりません</td>
                                        </tr>
                                    @endforelse
                                </tbody>
                            </table>
                ...
                <div class="flex justify-end border-t border-slate-200 bg-slate-50 px-4 py-3 dark:border-gray-700 dark:bg-gray-800">
                    <button type="button" wire:click="closeSupplierChangeModal" class="rounded-md border border-slate-300 bg-white px-3 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50 dark:border-gray-600 dark:bg-gray-900 dark:text-gray-200">変更せず閉じる</button>
                </div>
            </div>
        </div>
    @endif
