        ...
        <div class="bg-gray-50 dark:bg-white/5 rounded-lg p-3">
            <table class="w-full text-sm">
                <tbody class="divide-y divide-gray-200 dark:divide-white/10">
                    <tr>
                        <td class="py-1.5 text-gray-500 dark:text-gray-400 w-28">依頼倉庫</td>
                        <td class="py-1.5 font-medium text-gray-900 dark:text-white">{{ $satelliteWarehouseName }}</td>
                    </tr>
                    <tr>
                        <td class="py-1.5 text-gray-500 dark:text-gray-400">移動元倉庫</td>
                        <td class="py-1.5 font-medium text-gray-900 dark:text-white">{{ $hubWarehouseName }}</td>
                    </tr>
                    @if($deliveryCourseName && $deliveryCourseName !== '-')
                    <tr>
                        <td class="py-1.5 text-gray-500 dark:text-gray-400">配送コース</td>
                        <td class="py-1.5 text-gray-900 dark:text-white">{{ $deliveryCourseName }}</td>
                    </tr>
                    @endif
                    @if($contractorName && $contractorName !== '-')
                    <tr>
                        <td class="py-1.5 text-gray-500 dark:text-gray-400">発注先</td>
                        <td class="py-1.5 text-gray-900 dark:text-white">{{ $contractorName }}</td>
                    </tr>
                    @endif
                    <tr>
                    ...
                    <tr>
                        <td class="py-1.5 text-gray-500 dark:text-gray-400">実行CD</td>
                        <td class="py-1.5 font-mono text-gray-900 dark:text-white">{{ $batchCode }}</td>
                    </tr>
                    <tr>
                        <td class="py-1.5 text-gray-500 dark:text-gray-400">実行時刻</td>
                        <td class="py-1.5 text-gray-900 dark:text-white">{{ $batchCodeFormatted }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
        ...
        <div class="space-y-2">
            ...
            <div class="bg-slate-50 dark:bg-white/5 rounded-lg p-3 text-sm">
                ...
                <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-gray-600 dark:text-gray-400">
                    ...
                    <div class="flex justify-between"><span>自動発注数</span><span class="font-semibold text-gray-900 dark:text-white">{{ number_format($settingAutoOrderQuantity ?? 0) }}</span></div>
                    @if($isEditable ?? false)
