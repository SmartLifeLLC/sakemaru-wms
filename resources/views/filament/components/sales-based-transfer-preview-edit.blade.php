            ...
            <table class="min-w-full divide-y divide-slate-200 text-xs dark:divide-slate-700">
                <thead class="sticky top-0 z-10 bg-slate-100 text-slate-700 shadow-sm dark:bg-slate-800 dark:text-slate-200">
                    <tr>
                        ...
                        <th class="min-w-64 px-2 py-1.5 text-left font-semibold">商品名</th>
                        <th class="whitespace-nowrap px-2 py-1.5 text-left font-semibold">規格</th>
                        <th class="whitespace-nowrap border-l-2 border-slate-300 bg-amber-100 px-2 py-1.5 text-right font-semibold text-amber-900 dark:border-slate-600 dark:bg-amber-900/40 dark:text-amber-100">発注バラ</th>
                        <th class="w-8 whitespace-nowrap bg-amber-100 px-1 py-1.5 text-center font-semibold text-amber-900 dark:bg-amber-900/40 dark:text-amber-100"></th>
                ...
                <tbody class="divide-y divide-slate-100 dark:divide-slate-800">
                    <template x-for="(row, index) in rows" :key="row.item_code + '-' + index">
                        <tr class="odd:bg-white even:bg-blue-50/70 hover:bg-amber-50 dark:odd:bg-slate-900 dark:even:bg-slate-800/80 dark:hover:bg-amber-950/30">
                            ...
                            <td class="px-2 py-1.5 font-medium text-slate-900 dark:text-white" x-text="row.item_name"></td>
                            <td class="whitespace-nowrap px-2 py-1.5 text-slate-600 dark:text-slate-300" x-text="row.item_packaging || '-'"></td>
                            <td class="whitespace-nowrap border-l-2 border-slate-300 bg-amber-50 px-2 py-1.5 text-right dark:border-slate-600 dark:bg-amber-950/30">
                                <input
                                    type="text"
                                    inputmode="numeric"
                                    pattern="[0-9]*"
                                    ...
                                    class="w-20 rounded-md border-2 border-amber-300 bg-white px-2 py-0.5 text-right text-sm font-semibold text-slate-900 shadow-sm focus:border-amber-500 focus:ring-2 focus:ring-amber-200 dark:border-amber-700 dark:bg-gray-900 dark:text-gray-100 dark:focus:border-amber-500 dark:focus:ring-amber-900"
                                >
                            </td>
                            <td class="whitespace-nowrap bg-amber-50 px-1 py-1.5 text-center dark:bg-amber-950/30">
                                <button
                                    type="button"
                                    tabindex="-1"
