<div
    x-data="{
        ...
        normalize(row) {
            row.transfer_quantity = Math.max(0, Number(row.transfer_quantity || 0));
            this.sync();
        },
    }"
    x-init="sync()"
        ...
        <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700">
            <thead class="sticky top-0 z-10 bg-slate-100 text-xs font-semibold text-slate-700 shadow-sm dark:bg-gray-800 dark:text-gray-200">
                <tr>
                    ...
                    <th class="min-w-48 px-2 py-1.5 text-left">発注先</th>
                    ...
                    <th class="whitespace-nowrap border-l-2 border-slate-300 bg-amber-100 px-2 py-1.5 text-right text-amber-900 dark:border-slate-600 dark:bg-amber-900/40 dark:text-amber-100">発注数</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-800 dark:bg-gray-900">
                <template x-for="row in rows" :key="row.id">
                    <tr class="odd:bg-white even:bg-slate-50 hover:bg-blue-50 dark:odd:bg-gray-900 dark:even:bg-gray-800/60 dark:hover:bg-blue-950/40">
                        ...
                        <td class="px-2 py-1 text-xs" x-text="row.contractor_name"></td>
