<x-dynamic-component :component="$getFieldWrapperView()" :field="$field">
    <div
        x-data="{
            ...
            options: [
                { value: 'primary', label: '1次（波動集約）', icon: 'fa-layer-group', desc: '商品別の総数量一覧' },
                { value: 'secondary', label: '2次（作業者別）', icon: 'fa-user-hard-hat', desc: '棚番順＋納品先内訳' },
                { value: 'tertiary', label: '3次（納品先別仕分け）', icon: 'fa-store', desc: '配送コース別→納品先別' },
            ],
        }"
    ...
    >
        <template x-for="opt in options" :key="opt.value">
            <button
                ...
                :class="{
                    'border-blue-400 bg-blue-50 dark:border-blue-500 dark:bg-blue-900/30 ring-1 ring-blue-400': state === opt.value,
            ...
            >
                <i
                    :class="{
                        'text-blue-600 dark:text-blue-400': state === opt.value,
                ...
                <span
                    :class="{
                        'text-blue-700 dark:text-blue-300 font-bold': state === opt.value,
