...
>
    <div x-data="{
        ...
        init() {
            if (!Array.isArray(this.state)) this.state = this.state ? [this.state] : [];
            this.state = [...new Set(this.state.filter(Boolean))].sort();
            const base = this.state.length ? new Date(this.state[0]) : new Date();
        ...
        get displayDate() {
            ...
            if (this.state.length === 1) return this.formatDate(this.state[0]);
            return `${this.formatDate(this.state[0])} ほか${this.state.length - 1}日`;
        },
        ...
        isSelected(day) {
            if (!day) return false;
            return this.state.includes(this.dateValue(day));
        },
        ...
        addToday() {
            ...
            this.buildDays();
            const value = this.dateValue(t.getDate());
            if (!this.state.includes(value)) this.state = [...this.state, value].sort();
        },
    }"
    x-init="init()"
    ...
    >
        ...
        >
            <div class="flex items-center gap-2 min-w-0">
                ...
                <span class="truncate" x-text="displayDate || '日付を選択...'" :class="!displayDate && 'text-slate-400 dark:text-gray-500'"></span>
            </div>
            <span class="flex items-center gap-2 shrink-0">
                <span class="text-xs text-slate-400 dark:text-gray-500" x-text="state.length + '日'"></span>
        ...
        <div x-show="state.length" class="flex flex-wrap gap-1.5">
            <template x-for="value in state" :key="value">
                <button
                    type="button"
                    @click="removeDate(value)"
                    class="inline-flex items-center gap-1 rounded-md border border-blue-200 dark:border-blue-700 bg-blue-50 dark:bg-blue-900/30 px-2 py-1 text-xs font-medium text-blue-700 dark:text-blue-300 hover:bg-blue-100 dark:hover:bg-blue-900/50"
                >
        ...
        >
            ...
            <div class="flex items-center justify-between px-3 py-1.5 border-t border-slate-200 dark:border-gray-700 bg-slate-50 dark:bg-gray-800">
                <button type="button" @click="addToday()" class="text-xs text-blue-600 dark:text-blue-400 hover:underline font-medium">
                    今日を追加
                </button>
