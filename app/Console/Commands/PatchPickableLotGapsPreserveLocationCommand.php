...
class PatchPickableLotGapsPreserveLocationCommand extends Command
{
    protected $signature = 'stock:patch-pickable-lot-gaps-preserve-location
    ...
    protected $description = 'Patch real_stocks available gaps by preserving existing WMS lot locations.';
    ...
    public function handle(): int
    {
        ...
        foreach ($targets as $target) {
            $plan = $this->buildPatchPlan($target, $quantityFlag);
            $patchQty = (int) $target->missing_pickable_qty;
            $status = 'dry';
            $message = '';

            if ($patchQty <= 0) {
                $status = 'skipped';
                $message = 'no missing pickable quantity';
                $skipped++;
            } elseif (! $plan) {
                $status = 'skipped';
                ...
                $applied = $this->applyPatch((int) $target->real_stock_id, $quantityFlag, $plan);
                $patchQty = $applied['qty'];
                if ($patchQty <= 0) {
                    $status = 'skipped';
                    $message = 'no missing quantity after recheck';
                    $skipped++;
                } elseif ($applied['mode'] === 'update') {
                    $status = 'updated';
                    $updated++;
            ...
            $this->line(sprintf(
                '%s item=%s warehouse=%s real_stock_id=%d qty=%d mode=%s location=%s(%s) %s',
                strtoupper($status),
                $target->item_code,
            ...
            $rows[] = [
                ...
                'parent_current' => $target->current_quantity,
                'parent_reserved' => $target->reserved_quantity,
                ...
                'patch_qty' => $patchQty,
                'patch_lot_id' => $plan?->lot_id,
                'patch_location_id' => $plan?->location_id,
                'patch_floor_id' => $plan?->floor_id,
                'patch_location_code' => $this->formatLocationCode($plan),
        ...
        $this->info(($apply ? 'Patch complete' : 'Dry run complete')
            .": updated={$updated}, created={$created}, skipped={$skipped}, total_patch_qty={$totalPatchQty}");
    ...
    private function findTargets(?int $warehouseId, AvailableQuantityFlag $quantityFlag, ?int $itemId, int $limit): \Illuminate\Support\Collection
    {
        ...
        return $query
            ...
            ->havingRaw("{$pickableAvailableSql} < rs.available_quantity")
            ->orderByRaw("(rs.available_quantity - {$pickableAvailableSql}) DESC")
            ->limit($limit)
            ->get([
                ...
                DB::raw("(rs.available_quantity - {$pickableAvailableSql}) as missing_pickable_qty"),
            ]);
    }

    private function buildPatchPlan(object $target, AvailableQuantityFlag $quantityFlag): ?object
    {
    ...
    /**
     * @return array{mode: string, qty: int}
     */
    private function applyPatch(int $realStockId, AvailableQuantityFlag $quantityFlag, object $plan): array
    {
        return DB::connection('sakemaru')->transaction(function () use ($realStockId, $quantityFlag, $plan): array {
            ...
            $missingQty = (int) $stock->available_quantity - $pickableAvailable;
            if ($missingQty <= 0) {
                return ['mode' => 'skip', 'qty' => 0];
            }
            ...
            if ($plan->mode === 'update' && $plan->lot_id) {
                ...
                return ['mode' => 'update', 'qty' => $missingQty];
            }
            ...
            DB::connection('sakemaru')
                ...
                ->insert([
                    ...
                    'alert_date' => null,
                    'initial_quantity' => $missingQty,
                    'current_quantity' => $missingQty,
                    'reserved_quantity' => 0,
                    'status' => 'ACTIVE',
                    'created_at' => now(),
                    'updated_at' => now(),
            ...
            return ['mode' => 'create', 'qty' => $missingQty];
        });
    }
    ...
    private function writeCsv(array $rows): string
    {
        ...
        if (! $path) {
            $path = storage_path('reports/'.now()->format('Ymd-His').'-pickable-lot-gap-preserve-location-patch.csv');
        }
