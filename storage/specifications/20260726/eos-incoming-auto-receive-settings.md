...
| 自動完了 | 仕入自動連携後に、発注系は二週間前以前の入荷予定を欠品完了し、移動系は予定日を1日以上過ぎた入荷予定を入荷完了扱いにする |
...
7. 仕入自動連携後、発注系は二週間前の未確定入荷予定を欠品完了し、移動系は予定日を1日以上過ぎた未確定入荷予定を完了
...
仕入自動連携が完了した後に、未確定入荷予定を自動完了する。

発注系と移動系で完了条件と数量の扱いを分ける。

発注系:

- 基準日から14日前以前の未確定入荷予定を欠品完了する
...
- `status = CONFIRMED`
- `received_quantity` は現在値を維持
- `shortage_quantity = expected_quantity - received_quantity`
- `actual_arrival_date` は欠品完了処理日、または予定日
- `confirmed_by = 0`
...
この処理は仕入自動連携後に実行するため、自動完了処理そのものでは新たに仕入キューや移動キューを作らない。
...
- 二週間前欠品完了の対象はEOS送信済みに限定せず、全ての未確定入荷予定を対象にする
...
8. 二週間前欠品完了処理追加
   リスク: 高
   検証: 対象日以前の未確定だけ欠品完了され、対象外ステータスは変わらない
